...
import type { User, ClientUser, EmbedData } from 'discord.js';
...
class MiyakoClient extends Client {
  ...
  constructor(base?: string) {
    ...
    this.img = new ImgApiClient({
      port: process.env.IMGAPI_PORT as unknown as number,
  ...
  embed(user?: User | ClientUser | null, data: EmbedData = {}) {
  ...
  checkVote(user: User | string) {
    const userId = (user as User).id ?? user;

    return request(`https://top.gg/api/bots/${this.user!.id}/check`, {
      query: { userId },
      headers: { Authorization: process.env.DBL }
    })
      .then(({ body }) => body.json() as Promise<{ voted: unknown }>)

...
import type { Message, User, GuildMember, Role, Channel, TextChannel } from 'discord.js';
...
class Command extends Base {
  ...
  constructor(client: MiyakoClient, store: CommandStore, file: PieceLocation, options: CommandData = {}) {
  ...
  async execute(ctx: CommandContext) {
    try {
      ...
      if (ctx.text) await (ctx.channel as TextChannel).sendTyping();

      const options = ctx.slash ? (ctx.interaction.options as unknown as OptionResolver) : await this._parseArgs(ctx);
      ctx.options = options;
  ...
  getSlashCommandData(): SlashCommandBuilder | null {
    ...
    if (this.userPermissions.bitfield > 0n) {
    ...
    if (typeof this.options === 'function') {
      ...
      for (const data of this.options) {
        (builder as any)[`add${toProperCase(data.type)}Option`]((option: any) => {
  ...
  async _parseArg(ctx: CommandContext, i: number): Promise<unknown> {
    const message = ctx.message;
    const option = (this.options as CommandOption[])[i];
    const arg = ctx.args[i];
    const last = i === ((this.options as CommandOption[]).length - 1);
  ...
  async _parseArgs(ctx: CommandContext): Promise<CommandOptions> {
    const options: Record<string, unknown> = {};

    for (let i = 0; i < (this.options as CommandOption[]).length; i++) {
      const option = (this.options as CommandOption[])[i];
   ...
   */
  async verifyUser(msg: Message, user: string | undefined, required?: boolean): Promise<User | undefined> {
    ...
    const found = await this.client.users.fetch(match[1]).catch(() => null);
   ...
   */
  async verifyMember(msg: Message, member: string | undefined, required?: boolean): Promise<GuildMember | undefined> {
  ...
  async verifyChannel(msg: Message, channel: string | undefined, required = false): Promise<Channel | null | undefined> {
    ...
    if (!chan) throw (msg as unknown as LocalizedMessage).language.get('CHANNEL_NOT_FOUND');

    return chan;
  }

  verifyRole(msg: Message, rolename: string | undefined, required = false): Role | null {
    ...
    if (!role) throw (msg as unknown as LocalizedMessage).language.get('ROLE_NOT_FOUND');

    return role;
  }

  verifyInt(num: string | undefined, def?: number): number {
    if (typeof def === 'number' && !num) return def;
    const parsed = parseInt((num && num.replace(/,/g, '')) as string);
   ...
   */
  async check(ctx: CommandContext, options: OptionResolver): Promise<boolean | string> {
    return true;
  }
   ...
   * @abstract
   */
  async run(ctx: CommandContext, options: OptionResolver): Promise<unknown> {

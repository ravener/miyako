...
class Help extends Command {
  ...
  async run(ctx: CommandContext, options: OptionResolver) {
    ...
    const map: Record<string, string[]> = {}; // Map<Category, Array<Command.Name>>
    for (const command of this.store.values() as IterableIterator<Command>) {
      ...
      if (command.nsfw && !(ctx.channel as TextChannel).nsfw) continue;

...
class Conf extends Command {
  ...
  show(message, [key]) {
    ...
    return message.sendMessage(message.language.get("COMMAND_CONF_GET", path.piece.path, message.guild.settings.resolveString(message, path.piece)));
  }
  ...
  async set(message, [key, ...valueToSet]) {
    ...
    return message.sendMessage(message.language.get("COMMAND_CONF_UPDATED", key, message.guild.settings.resolveString(message, updated[0].piece)));
  }
  ...
  async reset(message, [key]) {
    ...
    if (errors.length) return message.sendMessage(errors[0]);
    if (!updated.length) return message.sendMessage(message.language.get("COMMAND_CONF_NOCHANGE", key));
    return message.sendMessage(message.language.get("COMMAND_CONF_RESET", key, message.guild.settings.resolveString(message, updated[0].piece)));
  }
}

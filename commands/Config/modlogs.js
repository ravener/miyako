...
class Modlogs extends Command {
  ...
  async enableModlogs(msg, channel, reply = false, force = false) {
    ...
    if(reply) return msg.send(`Enabled modlogs in channel ${channel}! Note: at the beginning nothing will be logged, so you have to enable which actions to log. To do that run \`${msg.guild.settings.prefix}modlogs enable <key>\` where key is one of \`${this.actions.join(", ")}\`. In addition you may use \`all\` to enable all actions. There is also a \`${msg.guild.settings.prefix}modlogs disable [key]\` to disable an action. If no action provided it disables modlogs, and the same actions plus all is also available for disable. To change modlogs channel run \`${msg.guild.settings.prefix}modlogs channel #some-modlogs\`. Lastly, run \`${msg.guild.settings.prefix}conf show modlogs\` to see all actions that is enabled/disabled. You may change an action using conf command as well, but modlogs command is the recommended way.`);
    return true;
  }
